...
df_source = spark.sql('''
select Distinct(Branch_ID),BranchName from parquet.`abfss://silver@caradedatalakemelvin.dfs.core.windows.net/carsales`;
''')
df_source.display()
...
if(spark.catalog.tableExists('cars_catalog.gold.dim_branch')):
    df_sink = spark.sql('''
    select Branch_Dim_Key,Branch_ID,BranchName
    from
    ...
    df_sink = spark.sql('''
    select 1 as Branch_Dim_Key,Branch_ID,BranchName
    from
...
df_filter = df_source.join(df_sink,df_source.Branch_ID == df_sink.Branch_ID,'left').select(df_source['Branch_ID'],df_source['BranchName'],df_sink['Branch_Dim_Key'])
...
df_filter_old = df_filter.filter(col('Branch_Dim_Key').isNotNull())
...
df_filter_new = df_filter.filter(col('Branch_Dim_Key').isNull()).select(df_filter['Branch_ID'],df_filter['BranchName'])
...
if incremental_flag == '0':
    ...
    max_value_df = spark.sql('select max(Branch_Dim_Key) from cars_catalog.gold.dim_branch')
...
# Takes the first column of that row ([0]), which is the result of MAX(Branch_Dim_Key)
...
df_filter_new = df_filter_new.withColumn('Branch_Dim_Key',max_value+monotonically_increasing_id())
...
if spark.catalog.tableExists('cars_catelog.gold.dim_branch'):
    delta_table = DeltaTable.forPath(spark,'abfss://gold@caradedatalakemelvin.dfs.core.windows.net/dim_branch')
    delta_table.alias('target')\
                .merge(df_final.alias('source'),'target.Branch_ID = source.Branch_ID')\
    ...
    df_final.write.format('delta')\
        .mode('overwrite')\
        .option('path','abfss://gold@caradedatalakemelvin.dfs.core.windows.net/dim_branch')\
        .saveAsTable('cars_catalog.gold.dim_branch')

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `cars_catalog`; select * from `gold`.`dim_branch` limit 100;