...
# MAGIC %md
# MAGIC ### Creating Dimesion date

# COMMAND ----------

df_source = spark.sql('''
select Distinct(Date_ID) from parquet.`abfss://silver@caradedatalakemelvin.dfs.core.windows.net/carsales`;
''')
df_source.display()
...
if(spark.catalog.tableExists('cars_catalog.gold.dim_date')):
    ...
    df_sink = spark.sql('''
    select 1 as Date_Dim_Key,Date_ID
    from
...
df_filter = df_source.join(df_sink,df_source.Date_ID == df_sink.Date_ID,'left').select(df_source['Date_ID'],df_sink['Date_Dim_Key'])
...
df_filter_old = df_filter.filter(col('Date_Dim_Key').isNotNull())
...
df_filter_new = df_filter.filter(col('Date_Dim_Key').isNull()).select(df_filter['Date_ID'])
...
if incremental_flag == '0':
    ...
    max_value_df = spark.sql('select max(Date_Dim_Key) from cars_catalog.gold.dim_date')
...
# Takes the first column of that row ([0]), which is the result of MAX(Date_Dim_Key)
...
df_filter_new = df_filter_new.withColumn('Date_Dim_Key',max_value+monotonically_increasing_id())
...
if spark.catalog.tableExists('cars_catelog.gold.dim_date'):
    delta_table = DeltaTable.forPath(spark,'abfss://gold@caradedatalakemelvin.dfs.core.windows.net/dim_date')
    delta_table.alias('target')\
                .merge(df_final.alias('source'),'target.Date_ID = source.Date_ID')\
    ...
    df_final.write.format('delta')\
        .mode('overwrite')\
        .option('path','abfss://gold@caradedatalakemelvin.dfs.core.windows.net/dim_date')\
        .saveAsTable('cars_catalog.gold.dim_date')

# COMMAND ----------

# MAGIC %sql
# MAGIC use catalog `cars_catalog`; select * from `gold`.`dim_date` limit 100;